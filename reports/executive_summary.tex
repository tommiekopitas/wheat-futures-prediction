...
This study investigates whether publicly available satellite-derived agricultural data can predict short-term European wheat futures price movements. Wheat is one of the most important agricultural commodities, and the Euronext Milling Wheat N2 futures contract serves as the primary price discovery mechanism for wheat in the European Union. Accurately forecasting price movements in this market carries significant practical value for producers, traders, farmers, corporate hedge funds, and policymakers across the agricultural supply chain and financial industry. 

Our study uses the CY-Bench benchmark dataset, the most comprehensive publicly available collection of sub-national crop yield indicators, covering 515 regions across France, Germany, and Poland, three of the largest wheat producers in the European Union. Features include meteorological variables, soil moisture, and satellite vegetation indices. These are combined with Euronext Milling Wheat N2 daily futures price data spanning 2003 to 2023.

Two prediction tasks are defined in this study. The primary task is the prediction of 5-day forward returns on wheat futures, evaluated on a test period covering 2020 to 2023. The secondary task is the prediction of annual regional wheat yields across the three countries at three lead times of end-of-season, mid-season, and quarter-season, then evaluated on a test period covering 2016 to 2020. The yield task serves as a validation of whether the CY-Bench features carry agricultural signals, which is necessary context for interpreting the futures results. We applied a range of modern machine learning methods, from simple statistical models like Ridge and Lasso regression, to ensemble tree-based methods of Random Forest and ExtraTrees, as well as advanced neural network models. The goal was to determine whether any approach could extract a reliable price signal from the agricultural data.

Our results reveal a clear gap between the two tasks. For wheat price prediction, all models across every level of complexity failed to extract a meaningful predictive signal from the agricultural data. This means that the agricultural data, despite containing substantial information about crop conditions, offered no advantage for forecasting short-term price movements in the futures market.

However, we found the same data proved useful for predicting crop yields, the task it was originally designed for, and is the secondary prediction task in our study. Our best models produced yield forecasts competitive with established academic benchmarks. ExtraTrees, our best-performing model, achieved a Normalised Root Mean Squared Error (NRMSE) of 13.00\% for Germany and 17.77\% for France at end-of-season, where a lower NRMSE indicates predictions closer to actual yields. Tree-based ensemble methods consistently outperformed linear models, confirming that the agricultural data contains useful crop production information.
